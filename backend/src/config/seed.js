...
const patientPassword = (patientId) => `${defaultPassword}-${patientId}-Patient!`;
...
async function seed() {
  ...
  const staffPasswordHash = await bcrypt.hash(defaultPassword, 12);
  try {
    ...
    for (const row of doctorRows) {
      ...
      await client.query(`INSERT INTO users (id, email, phone, password_hash, role, full_name)
        ...
        [userId, `doctor.${row.id}@fafcare.local`, row.phone, staffPasswordHash, row.full_name]);
    ...
    await client.query(`INSERT INTO users (id, email, phone, password_hash, role, full_name)
      ...
      [uuidFor('user', 'admin'), staffPasswordHash]);
